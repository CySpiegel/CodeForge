...
test("streamChat skips a malformed SSE frame instead of throwing a position error", async () => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = async (input: string | URL | Request) => {
    const url = input instanceof Request ? input.url : String(input);
    if (url.endsWith("/v1/models")) {
    ...
    const stream = new ReadableStream<Uint8Array>({
      start(controller) {
        const encoder = new TextEncoder();
        controller.enqueue(encoder.encode(`data: ${JSON.stringify({ choices: [{ delta: { content: "hel" } }] })}\n\n`));
        // A truncated/garbled frame (e.g. a fragment flushed after a dropped or quiet-cancelled stream)
        // must not throw "Unterminated string in JSON at position N" out of streamChat and lose the turn.
        controller.enqueue(encoder.encode(`data: {"choices":[{"delta":{"content":"BROKEN\n\n`));
        controller.enqueue(encoder.encode(`data: ${JSON.stringify({ choices: [{ delta: { content: "lo" } }] })}\n\n`));
        controller.enqueue(encoder.encode("data: [DONE]\n\n"));
        controller.close();
      }
...
test("streamChat emits reasoning deltas separately from content", async () => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = async (input: string | URL | Request) => {
    const url = input instanceof Request ? input.url : String(input);
    if (url.endsWith("/v1/chat/completions")) {
      const stream = new ReadableStream<Uint8Array>({
        start(controller) {
          const encoder = new TextEncoder();
          controller.enqueue(encoder.encode(`data: ${JSON.stringify({ choices: [{ delta: { reasoning_content: "let me " } }] })}\n\n`));
...
test("streamChat emits a rateLimit event before backing off a 429", async () => {
  const originalFetch = globalThis.fetch;
  let chatCalls = 0;
  globalThis.fetch = async (input: string | URL | Request) => {
    const url = input instanceof Request ? input.url : String(input);
