...
test("blocks public IP destinations even when allowlisted", () => {
  assert.equal(isUrlAllowed("https://203.0.113.10/v1", { allowlist: ["203.0.113.10"] }).allowed, false);
  assert.equal(isUrlAllowed("https://203.0.113.10/v1", { allowlist: ["203.0.113.0/24"] }).allowed, false);
});
