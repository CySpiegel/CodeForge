...
- Command system: implement built-ins (`/config`, `/settings`, `/model`, `/context`, `/memory`, `/compact`, `/clear`, `/resume`, `/doctor`, `/diff`) plus permission-mode commands (`/manual`, `/smart`, `/fullauto`, and aliases) and file-backed custom commands.
- Extension system: support skills and hooks as plain local files with frontmatter, plus local protocol server definitions.
- Subagents: run focused context workers for exploration, planning, review, verification, and implementation, returning summaries to the main session. Workers share the local endpoint, permission policy, and approval bridge, run under a bounded concurrency cap, and are seeded with the relevant learned lessons and skills.
- Learning: after a finished Agent-mode task, distil durable lessons from CodeForge's own work, store them as scoped memory rows, and inject the ranked-relevant ones into future prompts for both the main loop and sub-agents. Repeated successful procedures are proposed as reusable skills and recurring task types as review-only agents, with a periodic self-audit that dedups and prunes. Learning is fire-and-forget and never blocks or breaks a run.
...
- Implemented now (continued): the learning loop (distill durable lessons from finished Agent-mode tasks, rank and inject the relevant ones into the main loop and sub-agents, propose reusable skills and review-only agents, periodic self-audit to dedup/prune); focused multi-agent sub-agents (`spawn_agent`/`worker_output` with a bounded concurrency cap and built-in explore/plan/review/verify/implement kinds, lesson/skill-aware); deferred tool-schema loading (`tool_search`/`tool_list` load matching schemas for the next turn instead of front-loading every definition); local-protocol (MCP) resource/tool tools; durable local task tracking; the persona/"soul" injection; and code-intelligence tools (`code_hover`, `code_definition`, `code_references`, `code_symbols`) backed by VS Code language services.
- Next priority: wire the built-and-tested git worktree adapter into editing workers (currently isolation-ready but not connected), embeddings-based lesson retrieval (the `learning.embeddings.enabled` switch is declared but not yet wired), escalate-on-truncation retry, and a user-global persona/soul.
