...
This review is based on public Claude Code documentation and observable product behavior, not leaked or unlicensed source code. CodeForge should replicate the harness capabilities clean-room while keeping self-hosted OpenAI-compatible endpoints first class.
...
- Tool surface: the harness gives the model file operations, search, shell execution, web/MCP, code intelligence, subagents, and user-interaction tools.
...
- Extensibility: slash commands, skills, hooks, MCP servers, output styles, and subagents allow the harness to grow without hardcoding every workflow into the core loop.
...
- Tool registry: replace ad hoc actions with a typed registry that supports read, grep, glob, edit, write, bash, diagnostics, git, MCP, task, ask-user, and subagent tools.
...
- Command system: implement built-ins (`/config`, `/permissions`, `/model`, `/context`, `/compact`, `/clear`, `/resume`, `/init`, `/doctor`, `/diff`) and file-backed custom commands.
- Extension system: support skills and hooks as plain files with frontmatter, plus MCP server definitions.
...
- Implemented now: endpoint model discovery, selected-model setting, bottom-pinned composer, settings panel, context usage ring, model-driven `/compact`, and slash commands for `/context`, `/clear`, `/model`, and `/config`.
- Next priority: real session persistence/checkpoints and a richer permission engine. These two systems are the foundation for safer autonomy and resume/fork workflows.
- After that: itemized context accounting, `CODEFORGE.md`/`CLAUDE.md` loading, hook execution, MCP tools, and subagent isolation.
